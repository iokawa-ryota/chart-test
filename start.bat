...
start /min cmd /c "timeout /t 3 /nobreak > nul && start http://localhost:5000/login"

rem Rotate Flask session signing key on each startup to invalidate old login sessions.
set "FLASK_SECRET_KEY=startbat-%RANDOM%-%RANDOM%-%RANDOM%-%RANDOM%"
